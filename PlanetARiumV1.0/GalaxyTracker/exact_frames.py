...
def calculateTimes():
    ...
    print('Send 2000 frames time: ' + str(total_time - start_receiving_and_processing_frames))
    ...
    while fps < MAX_FRAMES:
        try:
            ...
            for barcode in decode(frame):
                ...
                b = np.array([barcode.polygon], np.int32)
                ...
                element = [myData, x_converted, y_converted, resized_radius]

                coordinates_list.append(element)

            if coordinates_list:
                json_data = {'coord': coordinates_list}
                position_sender.emit('qr', json_data)
                # print(json_data)
                last_sended = time.time()
                empty_sended = False
            elif (not coordinates_list) and (not empty_sended):
                actual_time = time.time()
                if (actual_time - last_sended) > 1:
                    json_data = {'coord': []}
                    position_sender.emit('qr', json_data)
                    # print('Enviada Ultima peticion')
                    empty_sended = True
            fps += 1
        except RuntimeError:
